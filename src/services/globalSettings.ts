...
export class GlobalSettingsService {
    ...
    private normalizeId(id: string): string {
        ...
        return `${paddedId.slice(0, 8)}-${paddedId.slice(
            8,
            12,
        )}-${paddedId.slice(12, 16)}-${paddedId.slice(16, 20)}-${paddedId.slice(
            20,
            32,
        )}`;
    }
    ...
    async update(
        id: string,
        body: Partial<Omit<GlobalSettingRecord, 'id'>>,
    ): Promise<GlobalSettingRecord | null> {
        const normalizedId = this.normalizeId(id);
        const updated = await withPg(async (c) => {
            ...
            const sql = `update ${
                this.schema
            }.fnd_global_settings set ${setClauses.join(
                ', ',
            )} where gs_id = $1 returning *`;
