...
// Alias controller for singular 'user' endpoint - queries acme.fnd_users table
@Controller('api/user')
class UserController {
    @Get()
    async list() {
        // Query the acme.fnd_users table directly
        const result = await withPg(async (c) => {
            const res = await c.query(
                `SELECT
                    ...
                    assigned_user_group as "assignedUserGroup",
    ...
    @Get(':id')
    async get(@Param('id') id: string) {
        // Query specific user by ID from acme.fnd_users
        const result = await withPg(async (c) => {
            const res = await c.query(
                `SELECT
                    id,
    ...
    async create(@Body() body: any, @Res() res: any) {
        try {
            ...
            const existingUser = await withPg(async (c) => {
                const res = await c.query(
                    `SELECT id FROM acme.fnd_users WHERE email_address = $1`,
                    [body.email],
                );
                return res.rows[0];
            });
            ...
            const created = await withPg(async (c) => {
                const res = await c.query(
                    `INSERT INTO acme.fnd_users (
                        account_id, enterprise_id, first_name, middle_name,
                        last_name, email_address, status, start_date, end_date,
                        password_hash, assigned_user_group, created_by
...
class UserGroupsController {
    ...
    async listByFilters(
        ...
        // Query the new fnd_user_groups table with filters
        const result = await withPg(async (c) => {
            const where: string[] = [];
            const params: any[] = [];
